...
const ContactMe = (props: Props) => {
    ...
    const onSubmit: SubmitHandler<Inputs> = (formData) => {
        let emailTo = 'haroonalbarx@gamil.com';

        //to redirect to send and email from the client
        //so now when you submit it opens up a mail client 
        window.location.href = `maio:?subject=${formData.subject}&body=Hai, my name is ${formData.name}. ${formData.message} (${formData.email})`
    };
    return (
        ...
        md:text-left max-w-7xl justify-evenly mx-auto items-center p-10'>
            ...
            <div className='flex flex-col mt-10 mb-10 space-y-3  p-20'>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className=' flex flex-col space-y-2 w-fit mx-auto' >
                    ...
                    <button onSubmit={handleSubmit(onSubmit)} type='submit' className='bg-teal-500 px-5 py-5 rounded-md text-black font-bold text-lg'>submit</button>
                </form>
            </div>
        </div>
    )
